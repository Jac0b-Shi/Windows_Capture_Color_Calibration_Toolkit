...
- 当前打包基线为 Windows 10 版本 1809 或更高版本（`TargetPlatformMinVersion=10.0.17763.0`）。
- WGC 需要 `Windows.Graphics.Capture` 支持。HDR/FP16 测量路径的最低 Windows 版本尚未完整验证。
- .NET SDK 10.0.x。
- 可从 NuGet 恢复 Windows App SDK 相关依赖。
- 推荐使用安装了 Windows 应用开发组件的 Visual Studio 运行 WinUI 项目。
- 建议在较新的 Windows 11 HDR 系统上进行开发和实验。
